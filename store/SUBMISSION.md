...
Upload chrome-web-store-0.6.22.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.22 preserves Instagram search results and scroll position when closing the viewer, skips previously watched Next targets, and waits for new results instead of restarting a one-keyword search. Existing comment-draft recovery, short-session pacing, confirmation checks and action limits remain intact. TikTok keeps comments disabled. Signup remains shelved behind a disabled bundled feature flag. Retain the existing beta and store-review caveats; see work_log.txt for verification results.
