...
Upload chrome-web-store-0.6.21.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.21 preserves exact untouched drafts across Instagram textarea replacements, fixes short-session engagement timing and reserves time for confirmation. It recognizes usable caption sentences when the niche appears elsewhere in the caption. Includes missing-post recovery, no-replay action allowances and read-only follow confirmation. A real Instagram session completed with scrolling, a confirmed like, follow and comment. TikTok keeps comments disabled. Signup remains shelved behind a disabled bundled feature flag. Retain the existing beta and store-review caveats; see work_log.txt for verification results.
