...
Upload chrome-web-store-0.6.19.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.19 adds a read-only confirmation for Instagram follows: if the viewer cannot confirm a follow, a temporary inactive tab loads the same post and checks the exact author's Following or Requested state. The tab closes after the check; the follow is never retried and missing confirmation does not stop the session. Includes the 0.6.18 comment-draft recovery. TikTok keeps comments disabled. Signup remains shelved behind a disabled bundled feature flag. No live high-engagement session has been run on this exact release; retain the existing beta and store-review caveats.
