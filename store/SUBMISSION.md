...
Upload chrome-web-store-0.6.20.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.20 adds recovery for missing posts, empty searches and temporary page-loading failures. Uncertain engagement consumes its allowance without being retried or counted as confirmed. Includes read-only follow confirmation and comment-draft recovery. TikTok keeps comments disabled. Signup remains shelved behind a disabled bundled feature flag. Retain the existing beta and store-review caveats; see work_log.txt for verification results.
