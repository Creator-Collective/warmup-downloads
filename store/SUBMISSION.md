...
Upload chrome-web-store-0.6.23.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.23 makes like pacing target-driven without random skips, reserves time for final confirmations, and holds suitable unliked posts through short cooldowns. Watches and breaks account for the next planned like. Follows, comments and scrolling remain enabled, with existing action limits and confirmation guards. The one-keyword search-progress and repeated-post repairs remain intact. TikTok keeps comments disabled. Signup remains shelved behind a disabled bundled feature flag. Retain the existing beta and store-review caveats; see work_log.txt for verification results.
