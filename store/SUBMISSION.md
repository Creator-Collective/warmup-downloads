...
Upload chrome-web-store-0.6.6.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Signup additions require disclosure of TikTok host access, the fixed account-email service endpoint on trycreatorcollective.com, temporary password handling, persistent local mailbox access keys, configured birthday filling/local retention, and private-window recovery for logged-in Instagram browsers. Version 0.6.6 is not live-verified; do not submit screenshots or claims of completed live signup until verified.
