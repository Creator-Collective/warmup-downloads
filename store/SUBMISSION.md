...
Upload chrome-web-store-0.6.24.zip. Add store screenshots of the actual dashboard and running session only after browser verification. Store review may require changes; do not represent this as approved.

Version 0.6.24 keeps comment text, post links, author, time and confirmation status in a dedicated history for the current browser session. History survives panel reopening and session completion, and clears on a new session. It is shown locally in the runner, panel and connected dashboard; no server storage or analytics is added. Drafts are not presented as posted comments. Earlier text cannot be backfilled. Like pacing, search-progress repairs and all existing action guards remain intact. TikTok keeps comments disabled. Signup remains shelved. Retain the existing beta and store-review caveats; see work_log.txt for verification results.
