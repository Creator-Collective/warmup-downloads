...
if (!setup.includes(`data-latest-version="${version}"`) || !setup.includes(`id="latest-version">${version}<`) || !setup.includes(`id="walkthrough-version">${version}<`)) errors.push('setup version comparison is stale');
for (const asset of ['setup.css', 'platform-instagram.svg', 'platform-tiktok.svg']) if (!read('.vercelignore').toString().split(/\r?\n/).includes(`!${asset}`)) errors.push(`missing public asset: ${asset}`);
...
for (const file of ['comment-history.js','session-results.js','dashboard.js','select-ui.js','signup-ui.js','phone-ui.js','plan.js','dashboard.css','inter.woff2','cc-logo.webp','platform-instagram.svg','platform-tiktok.svg','INTER-LICENSE.txt']) {
