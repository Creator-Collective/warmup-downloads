## 0.6.58 no instagram behavior changes, prepares a separate tiktok test build, September 25

All 833 automated checks pass. Both 36-file Instagram archives and the packaged dashboard mirrors match source; against 0.6.57 the archive has the same file list, the manifest differs only in its version, and sidepanel.html and runner.html are unchanged. Instagram pins recorded before any edit still pass unchanged: 36 golden 40-minute goals runs (3 caption scenarios x 12 seeds, exact stats and a digest of the ordered trace), runner transitions, draft checks and confirmation cadence, background gates and messages, and dashboard payloads on the hosted page and side panel.

TikTok simulation (real plan, writer, engine, runner confirmation and tiktok.js on a DOM fixture, finite results, 2-6 s confirmation tabs and search loads, the same scenarios, seeds and 40 min default targets as the Instagram baseline): 58/60 likes, 17.3/18 follows, 6.1/7 comments on average, equal to Instagram's 58/17.3/6.1, with every scenario at 90% or more, about 7 keyword searches, at most 2 actions per post, at most 6 action starts per minute and 0 repeated actions or comments. The TikTok keyword window moved from 2 to 6 minutes for this: with 2 minutes corpus comments reached only 3.25 against Instagram's 4.25.

The TikTok test build 0.6.58.1 (dist/creator-collective-tiktok-test-0.6.58.1.zip, not committed or deployed) was rebuilt byte for byte and unzipped: TikTok-only hosts, no content scripts or bridges, features platforms ['tiktok'] with test tools, and every other file byte-equal to browser-extension/. The earlier 0.6.57.1 test build was loaded unpacked in Chrome for Testing 153 to check the fixed id, the badge and the side panel (see TESTING-TIKTOK.md). No installed Instagram extension reload, Instagram or TikTok session, or social action was performed for this release. Nothing TikTok is live-verified: whether TikTok registers the build's clicks, keeps likes and follows, accepts pasted comments, sets html lang, or restores unsent drafts is what the tester protocol in TESTING-TIKTOK.md checks.
